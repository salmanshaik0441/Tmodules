resource "aws_kms_key" "s3_kms_key" {
  description              = "Key for ${var.business_event_lamda_bucket_name} state"
  ...
  tags = {
    Name = "${var.business_event_lamda_bucket_name} s3 bucket key"
  }
}

resource "aws_kms_alias" "s3_kms_key_alias" {
  name          = "alias/${var.business_event_lamda_bucket_name}"
