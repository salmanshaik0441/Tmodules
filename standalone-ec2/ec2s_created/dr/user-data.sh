...
# RESHOST=$(nslookup bifrost.orbit.prod.vodacom.co.za $NSERVER | grep Address | tail -n1 | awk '{print $2}' | cut -d'#' -f1)
# echo $RESHOST bifrost.orbit.prod.vodacom.co.za >> /etc/hosts
# RESHOST=$(nslookup artifactory.orbit.prod.vodacom.co.za $NSERVER | grep Address | tail -n1 | cut -d' ' -f2)
# echo $RESHOST artifactory.orbit.prod.vodacom.co.za >> /etc/hosts

# Adding proxy to yum.conf
echo proxy=http://bifrost.orbit.prod.vodacom.co.za:3128 >> /etc/yum.conf
...
pip3 install markupsafe==2.0.1 jinja2==3.0

# Installing and configuring salt-minion
wget -O install_salt.sh "http://artifactory.orbit.prod.vodacom.co.za/ui/api/v1/download?repoKey=local-dists&path=salt%2Fbootstrap"
